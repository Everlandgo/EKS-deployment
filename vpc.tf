...
# Target Group for NodePort 30080 (all services routed via Ingress)
resource "aws_lb_target_group" "eks_nodeport_tg" {
  name        = "community-eks-tg"
  port        = 30080
  protocol    = "HTTP"
  vpc_id      = aws_vpc.community_vpc.id
  target_type = "ip"
...
# ALB HTTP Listener forwarding to NodePort 30080
resource "aws_lb_listener" "http" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.eks_nodeport_tg.arn
  }
}
